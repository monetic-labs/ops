...
export function evaluateTransferMethod(
    ...
    const rule = preferences.decision_rules.find(rule => 
        evaluateCondition(rule.condition, amount)
    );

    // Filter methods based on speed expectations
    const viableMethods = availableMethods.filter(method => 
        method.speed <= parseInt(preferences.parameters.speed_expectations.max_acceptable_time)
    );
    ...
    switch (rule?.action) {
        ...
        case 'evaluate_fee_threshold':
            return getOptimalMethodByFeePercentage(
                viableMethods, 
                amount,
                rule.max_fee_percentage || 0
            );
...
function getOptimalMethodByFeePercentage(
    ...
    const validMethods = methods.filter(method => {
        const effectiveFee = method.feeType === 'percentage' 
            ? (method.fee / 100) * amount 
            : method.fee;
    ...
    return validMethods.sort((a, b) => {
        if (Math.abs(a.speed - b.speed) <= 1) { // If speeds are within 1 minute
...
function suggestMethodWithConfirmation(
    ...
    const effectiveFee = fastestMethod.feeType === 'percentage'
        ? (fastestMethod.fee / 100) * transactionAmount
        : fastestMethod.fee;
    
    if (effectiveFee > costTolerance.max_premium_absolute) {
        // Find next best method within tolerance
        return sortedMethods.find(method => {
            const fee = method.feeType === 'percentage'
                ? (method.fee / 100) * transactionAmount
                : method.fee;
