...
test.describe("Mode Switcher Component", () => {
  ...
  test("syncs with URL parameters", async ({ page }) => {
    ...
    const verifyTabStates = async (activeMode: "support" | "agent") => {
      ...
      await expect(supportTab).toHaveAttribute("aria-selected", activeMode === "support" ? "true" : "false");
      await expect(agentTab).toHaveAttribute("aria-selected", activeMode === "agent" ? "true" : "false");
    };
