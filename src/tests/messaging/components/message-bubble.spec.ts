...
import { AgentChatContext, AgentMessageService, Message } from '@/types/messaging';
import { addTestMessage, addTestMessages, createEmptyAgentContext, injectTestUtils } from '@/tests/helpers/test-utils';
...
test.describe('Message Bubble Component', () => {
    ...
    test('maintains correct message order', async ({ page }) => {
        ...
        const messageElements = await page.$$('[data-testid^="message-"]:not([data-testid$="-content"]):not([data-testid$="-status"])');
  ...
  test('sanitizes message content correctly', async ({ page }) => {
    ...
    const hasScript = await messageContent.evaluate(element => 
        element.innerHTML.includes('script') || element.innerHTML.includes('xss')
    );
